...
// Code base on: https://github.com/studiosi/OpenCVDeviceEnumerator
// Some adjustments were needed due to the QT compatibility.
...
std::map<int, Device> DeviceEnumerator::getDevicesMap(const GUID deviceClass)
{
    ...
    if (SUCCEEDED(hr)) {
        ...
        while (pEnum->Next(1, &pMoniker, NULL) == S_OK) {
            ...
            var = {}; // We clean the variable in order to read the next value
...
{
    size_t wslen = wcslen(bstr);
    return ConvertWCSToMBS((wchar_t*)bstr, static_cast<long>(wslen));
}
